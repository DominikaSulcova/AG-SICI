%% AG-SICI: STRENGTH OF THE RESPONSE
...
for p = 1:length(position)
    for c = 1:length(current)
        ...
        for e = 1:size(AGSICI_data, 5)
            for t = 1:size(AGSICI_data, 6)
                data_gfp(p, c, e, t) = mean(squeeze(data_mean(p, c, :, e, t)));
            end
        end 
    end
end
...
%calculate GFP and plot separate figures
for p = 1:length(position)
    for c = 1:length(current)
        % calculate GFP (exclude target channel)
        GFP(p, c, :) = std(squeeze(data_gfp(p, c, 1:size(data_gfp, 3) - 1, :)), 1); 
...
for p = 1:length(position)
    for s = 1:size(AGSICI_data, 4)   
        GFP_position(p, s, :) = mean(squeeze(GFP(p, :, s, :)), 1);
    end
end
clear p s 

% average data across subjects                
data_visual = squeeze(mean(squeeze(GFP_position(:, :, :)), 2));
...
for p = 1:length(position)
    for s = 1:size(AGSICI_data, 4)    
        AUC_position(p, s) = trapz(squeeze(GFP_position(p, s, x_start:x_end)));
    end
end
